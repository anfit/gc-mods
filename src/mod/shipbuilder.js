...
app.mod.shipbuilder = {
	...
	plugin: function () {
		...
		if (gc.location.match(/com_ship.*shiptype/)) {
			var id = new String(gc.location).replace(/.*shiptype=/, '') * 1;
			var ship = allShips[id];
			//console.log(ship);
			if (!ship) {
				ship = {};
